// scoring.ts — ported verbatim from data.js
// IDS_WORKFLOWS, IDS_RESPONSE, IDS_COVERAGE, scoreWorkflows
...
export interface Workflow {
  ...
  boost_coverage_gap?: number;
  boost_speed?: number;
  boost_medium_volume?: number;
  boost_high_value?: number;
  boost_triage?: number;
  boost_fast_responder?: number;
  boost_high_volume?: number;
  boost_medium_value?: number;
  startHere?: (i: Inputs) => boolean;
}
...
export function scoreWorkflows(inputs: Inputs): Workflow[] {
  let candidates = WORKFLOWS.filter(w => w.industry === inputs.industry);
